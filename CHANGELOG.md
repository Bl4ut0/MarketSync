...
- **Changed**: Confirmed the 1.15.9 Edit Mode/API transition follows the same addon API family as TBC Anniversary 2.5.6. MarketSync's Settings registration, guarded legacy fallbacks, custom frames, minimap alerts, chat transport, and Auctionator full-scan hooks remain compatible on both clients.

## [0.8.0-rc1] - 2026-07-19
### Added
- **Added**: A protocol-revision compatibility framework separates the addon's release version from its wire format. Releases on the same protocol revision may communicate; incompatible clients remain control-plane-only, advertise their version, and receive a clear update notice instead of beginning a transfer that cannot finish.
- **Added**: Session identifiers, ordered chunk numbers, and final sequence metadata now frame each guild broadcast without requiring a completion ACK from every listener.
- **Added**: Notification urgency, minimap flashing, in-addon alert banners, and per-item sound selection. Sound remains disabled by default; on TBC, 0% is a MarketSync hard mute and non-zero playback follows the game's Master volume because `PlaySound` exposes no per-alert gain.

### Changed
- **Changed**: Guild synchronization now elects the exact advertiser of the requested scan and completes one deterministic, sequential broadcast before considering the newest queued advertisement. Parallel data dumps and receiver retries can no longer block the guild queue.
- **Changed**: Main and Neutral AH sync remain independently configurable but share one physical transmitter. Main wins an idle contention window when both scopes are waiting; whichever scope owns the active session then finishes without interruption inside the same total addon-message budget.
- **Changed**: Bulk payloads use compact base-36 delta records so already-known history points are not resent. This reduces transfer size with minimal Lua CPU overhead.
- **Changed**: Incoming data is staged until the matching session finishes. An incomplete or rejected session is purged, never advances freshness, and can never become an outbound broadcast source; the client simply waits for a later normal guild broadcast.
- **Changed**: Existing pre-RC observations remain available for local browsing, but the verified-source migration fails closed: a new successful full scan or complete protocol-2 download is required before that client advertises market data.
- **Changed**: This release candidate is validated for WoW TBC Anniversary 2.5.6 and declares Interface 20506 support while retaining Interface 20505 compatibility.

### Fixed
- **Fixed**: Prospecting, milling, disenchanting, and crafting profitability now use consistent net Auction House proceeds after the 5% sale cut, preventing inflated maximum-buy and profit figures.
- **Fixed**: Variable-yield crafting recipes now use their expected output quantity instead of treating the minimum return as the guaranteed result.
- **Fixed**: Neutral AH freshness is updated only after a real neutral scan is captured; merely closing the Auction House no longer makes stale neutral data appear fresh.
- **Fixed**: Main and Neutral price updates now pass through the shared notification evaluator, and alert thresholds retain their precise copper values when edited.
- **Fixed**: Thirty-minute scan buckets are anchored to Auctionator's scan-day epoch, preventing timezone-dependent revision drift in synchronized history.
