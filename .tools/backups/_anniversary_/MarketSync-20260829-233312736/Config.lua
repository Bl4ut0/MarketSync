...
-- Main prefix for low-rate protocol coordination (ADV, PULL, and version state).
C_ChatInfo.RegisterAddonMessagePrefix(PREFIX)

-- Protocol 2 sends BEGIN/DATA/END sequentially on MSyncD1.  The retired data
-- prefixes remain registered during the release-candidate rollout so messages
-- from older installations fail closed without producing unknown-prefix noise.
...
function MarketSync.InitializeDB()
    if not MarketSyncDB then
        MarketSyncDB = {
            ...
            EnableMinimapAlerts = true,
            EnableRaidWarningAlerts = true,
    ...
    -- Sound is opt-in. Existing explicit preferences are preserved during upgrades.
    if MarketSyncDB.EnableNotificationSounds == nil then MarketSyncDB.EnableNotificationSounds = false end
    if MarketSyncDB.EnableMinimapAlerts == nil then MarketSyncDB.EnableMinimapAlerts = true end
    if MarketSyncDB.EnableRaidWarningAlerts == nil then MarketSyncDB.EnableRaidWarningAlerts = true end
    ...
    if MarketSyncDB.NotificationVolume == nil then MarketSyncDB.NotificationVolume = 1.0 end
    ...
    if realmDB then
        ...
        if realmDB.NotificationSettings.urgentMinIntervalSec == nil then
            realmDB.NotificationSettings.urgentMinIntervalSec = 10
        end
    end
end
...
function MarketSync.FormatRealmTime(timestamp, formatStr)
    ...
    local adjustedTime = ts + (diffMins * 60)
...
function MarketSync.FormatRealmDateString(timestamp)
    ...
    if d1.year == d2.year then
        ...
        return date("%b %d, %Y at %H:%M RT", adjustedTime)
    end
end
...
-- Plain, lossless money text for editable fields (for example "12g 34s 56c").
function MarketSync.FormatMoneyPlain(amount)
    local copperTotal = math.max(0, math.floor(tonumber(amount) or 0))
...
-- Accept exact WoW money strings, or a bare/decimal number interpreted as gold.
function MarketSync.ParseMoneyToCopper(value)
    local raw = tostring(value or ""):lower():gsub(",", ".")
    raw = raw:gsub("%s+", "")
    if raw == "" then return 0 end

    local hasDenomination = raw:find("g", 1, true)
        or raw:find("s", 1, true)
        or raw:find("c", 1, true)
    if not hasDenomination then
        local gold = tonumber(raw)
        if not gold or gold <= 0 then return 0 end
        return math.max(0, math.floor((gold * 10000) + 0.5))
    end

    local gold = tonumber(raw:match("([%d%.]+)g")) or 0
    local silver = tonumber(raw:match("([%d%.]+)s")) or 0
    local copper = tonumber(raw:match("([%d%.]+)c")) or 0
    return math.max(0, math.floor((gold * 10000) + (silver * 100) + copper + 0.5))
end

-- Backward-compatible name used by notification UIs.
MarketSync.ParseGoldToCopper = MarketSync.ParseMoneyToCopper
