...
local function ScanDayOffsetToBucket(scanDay, bucketOffset)
    local dayZero = Auctionator and Auctionator.Constants and Auctionator.Constants.SCAN_DAY_0
    if dayZero then
        return math.floor(tonumber(dayZero) / 1800) + ((tonumber(scanDay) or 0) * 48) + (tonumber(bucketOffset) or 0)
    end
    -- Fallback keeps the relative day aligned with the current absolute bucket.
    local currentBucket = MarketSync.GetCurrentBucket()
    local currentDay = MarketSync.GetCurrentScanDay()
    return (currentBucket - (currentDay * 48) - (currentBucket % 48))
        + ((tonumber(scanDay) or 0) * 48) + (tonumber(bucketOffset) or 0)
end

local function BucketToScanDay(bucket)
    local dayZero = Auctionator and Auctionator.Constants and Auctionator.Constants.SCAN_DAY_0
    if dayZero then
        return math.floor(((tonumber(bucket) or 0) - math.floor(tonumber(dayZero) / 1800)) / 48)
    end
    local currentBucket = MarketSync.GetCurrentBucket()
    local currentDay = MarketSync.GetCurrentScanDay()
    return currentDay - math.floor((currentBucket - (tonumber(bucket) or 0)) / 48)
end
...
function MarketSync.SnapshotPersonalScan(options)
    ...
    local bucketOffset = bucketID - ScanDayOffsetToBucket(today, 0)
    if bucketOffset < 0 or bucketOffset >= 48 then
        Debug("Personal snapshot skipped: current bucket could not be aligned to Auctionator's scan-day epoch")
        return 0, 0, 0
    end
