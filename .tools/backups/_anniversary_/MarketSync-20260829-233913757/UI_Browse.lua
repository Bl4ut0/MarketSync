...
local function BuildIndexEntry(dbKey, itemID, data, sourceMode)
    ...
    if sourceMode == "personal" then
        ...
        if pTime and pTime > 0 then
            local scanDayOfPTime = (Auctionator and Auctionator.Constants and Auctionator.Constants.SCAN_DAY_0)
                and math.floor((pTime - Auctionator.Constants.SCAN_DAY_0) / 86400) or currentDay
            if dbDay == scanDayOfPTime or age == 0 then
                exactTime = pTime
            end
        end
...
local function BuildSearchIndex(callback)
    ...
    local function ProcessPendingBatch()
        ...
        for dbKey, itemID in pairs(PersonalPending) do
            ...
            local pStore = MarketSync.GetRealmDB().PersonalData
            local data = (pStore and pStore[dbKey]) or Auctionator.Database.db[dbKey]
            if data then
                local entry = BuildIndexEntry(dbKey, itemID, data, "personal")
                if entry then
                    PersonalIndex[dbKey] = entry
...
function MarketSync.CreateBrowsePanel(parent, dataSourceName)
    ...
    -- --- SYNC LABEL (inside the bottom gold bar, left side) ---
    panel.syncLabel = panel:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
    panel.syncLabel:SetPoint("BOTTOMLEFT", parent, "BOTTOMLEFT", 30, 20)
    panel.syncLabel:SetJustifyH("LEFT")

    -- --- STATUS TEXT (anchored directly after sync label) ---
    panel.statusText = panel:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
    panel.statusText:SetPoint("LEFT", panel.syncLabel, "RIGHT", 4, 0)
    ...
    panel:SetScript("OnShow", function(self)
        ...
        if self.dataSource == "personal" then
            ...
            if personalTime > 0 then
                local timeStr = MarketSync.FormatRealmDateString(personalTime)
        ...
        elseif self.dataSource == "guild" then
            ...
            local personalTime = MarketSyncDB and MarketSync.GetRealmDB().PersonalScanTime or 0
            if personalTime > latestTime and personalTime > 0 then
                local timeStr = MarketSync.FormatRealmDateString(personalTime)
                local myName = UnitName("player") or "You"
                self.syncLabel:SetText("|cff00ff00Latest Data:|r")
                self.statusText:SetText("|cffffd700" .. timeStr .. " (" .. myName .. ")|r")
            elseif latestUser and latestTime > 0 then
                local timeStr = MarketSync.FormatRealmDateString(latestTime)
        ...
        elseif self.dataSource == "neutral" then
            ...
            local neutralTime = MarketSync.GetRealmDB().NeutralScanTime
            if neutralTime and neutralTime > 0 then
                self.syncLabel:SetText("|cff00ccffLast Neutral Scan:|r")
                self.statusText:SetText("|cffffd700" .. MarketSync.FormatRealmDateString(neutralTime) .. "|r")
            else
                self.syncLabel:SetText("")
