...
local function ParseGoldToCopper(text)
    ...
    local gold = tonumber(TrimText(text):gsub(",", "."))
...
local function FormatGoldInput(copper)
    ...
    return tostring(math.max(0, tonumber(copper) or 0) / 10000)
end
...
local function ResolveRequestVisual(req)
    ...
    elseif matchType == "dbKey" and MarketSync.ParseItemIDFromDBKey then
        itemID = MarketSync.ParseItemIDFromDBKey(matchValue)
...
local function ResolveRequestDBKey(req, itemID)
    if req and req.matchType == "dbKey" and req.variantMode == "exact_key" then
        return req.matchValue
    end
    return itemID and tostring(itemID) or (req and req.matchValue)
end
...
local function GetAuctionatorDataByKey(dbKey)
    ...
    if not data then
        ...
        if numericKey then data = db[numericKey] end
    end
    return data
end
...
function MarketSync.CreateNotificationsPanel(parent)
    ...
    local urgentCheck = CreateFrame("CheckButton", nil, leftTopBox, "UICheckButtonTemplate")
    urgentCheck:SetSize(22, 22)
    urgentCheck:SetPoint("TOPLEFT", leftTopBox, "TOPLEFT", 8, -168)
    urgentCheck.text:SetText("Urgent")
    urgentCheck.text:ClearAllPoints()
    urgentCheck.text:SetPoint("LEFT", urgentCheck, "RIGHT", 2, 0)
    urgentCheck:SetChecked(false)
    panel.manualUrgentDirty = false
    urgentCheck:SetScript("OnClick", function()
        panel.manualUrgentDirty = true
        urgentCheck.text:SetText("Urgent")
    end)

    local btnAdd = CreateFrame("Button", nil, leftTopBox, "UIPanelButtonTemplate")
    btnAdd:SetSize(LEFT_W - 16, 20)
    btnAdd:SetPoint("TOPLEFT", leftTopBox, "TOPLEFT", 8, -192)
    ...
    importFallbackBox:SetNumeric(false)
    ...
    local function ApplyManualSelectionState()
        ...
        local sharedUrgent = SharedValue(function(req)
            return req.urgent == true
        end)
        panel.manualUrgentDirty = false
        urgentCheck:SetChecked(sharedUrgent == true)
        urgentCheck.text:SetText(sharedUrgent == nil and "Urgent (mixed)" or "Urgent")
    ...
    local function RefreshRows()
        ...
        for i = 1, ROWS_PER_PAGE do
            ...
            if req then
                ...
                local sourceText = TrimText(req.importSource or "-")
                if req.urgent then sourceText = "! " .. sourceText end
                        ...
                        UIDropDownMenu_Initialize(menuFrame, function(_, level)
                                    ...
                                    info.func = function()
                                        ...
                                        if sID and sID > 0 then
                                            if MarketSync.PlayNotificationSound then
                                                MarketSync.PlayNotificationSound(sID, true)
                                            end
                                        end
                                    end
                                    UIDropDownMenu_AddButton(info, level)

                                    for _, sound in ipairs(MarketSync.StandardSounds or {}) do
                                        if tonumber(sound.id) and tonumber(sound.id) > 0 then
                                            local selectedSound = sound
                                            info = UIDropDownMenu_CreateInfo()
                                            info.text = selectedSound.name
                                            info.notCheckable = false
                                            info.checked = (currentSound == selectedSound.id)
                                            info.func = function()
                                                if not MarketSyncDB.PerNotificationSounds then MarketSyncDB.PerNotificationSounds = {} end
                                                MarketSyncDB.PerNotificationSounds[rowReqID] = selectedSound.id
                                                RefreshRows()
                                            end
                                            UIDropDownMenu_AddButton(info, level)
                                        end
                            ...
                            info.func = function()
                                ...
                                if hItemID and MarketSync.ShowItemDetail then
                                    MarketSync.ShowItemDetail(hItemID, rowReqID)
                                end
                            end
                            UIDropDownMenu_AddButton(info, level)
                            ...
                            info.func = function()
                                if MarketSync.ShowItemHistory then
                                    ...
                                    local hDBKey = ResolveRequestDBKey(req, hItemID)
                                    local hPrice = nil
                                    local d = GetAuctionatorDataByKey(hDBKey)
                                    hPrice = d and d.m
                            ...
                            -- Urgent alerts bypass the normal cooldown, while retaining
                            -- the short anti-spam safety interval from NotificationSettings.
                            info = UIDropDownMenu_CreateInfo()
                            info.text = payload.urgent and "Disable Urgent" or "Enable Urgent"
                            info.notCheckable = true
                            info.func = function()
                                payload.urgent = not payload.urgent
                                if MarketSync.UpsertNotificationRequest then
                                    MarketSync.UpsertNotificationRequest(payload)
                                end
                                RefreshRows()
    ...
    btnAdd:SetScript("OnClick", function()
        ...
        if #selected > 0 then
            ...
            for _, req in ipairs(selected) do
                ...
                if panel.manualUrgentDirty then
                    payload.urgent = urgentCheck:GetChecked() and true or false
                end

                local saved = MarketSync.UpsertNotificationRequest(payload)
                if saved then
                    updated = updated + 1
                    nextSelectedIDs[saved.id] = true
                end
            end
            panel.selectedIDs = nextSelectedIDs
    ...
    panel:SetScript("OnShow", function()
        ...
        if soundCheck then
            soundCheck:SetChecked(MarketSyncDB and MarketSyncDB.EnableNotificationSounds == true)
        end
        RefreshImportDropdown()
        RefreshRows()
        SetStatus("")
