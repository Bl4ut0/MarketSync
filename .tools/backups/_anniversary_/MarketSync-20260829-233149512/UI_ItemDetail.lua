...
function MarketSync.CreateItemDetailPanel(parent)
    ...
    local urgentCheck = CreateFrame("CheckButton", nil, alertsBox, "InterfaceOptionsCheckButtonTemplate")
    urgentCheck:SetPoint("TOPLEFT", 230, -55)
    urgentCheck.Text:SetText("Urgent")
    panel.urgentCheck = urgentCheck
    ...
    btnSave:SetScript("OnClick", function()
        ...
        local req = {
            id = panel.currentNotificationRequestID,
            matchType = panel.currentNotificationMatchType or "itemID",
            matchValue = panel.currentNotificationMatchValue or panel.currentItemItemID,
            displayName = panel.currentItemName,
            thresholdCopper = tVal,
            scope = panel.currentNotificationScope or "all",
            variantMode = panel.currentNotificationVariantMode or "any_suffix",
            cooldownSec = cVal,
        ...
        if MarketSync.UpsertNotificationRequest then
            ...
            if saved then
                panel.currentNotificationRequestID = saved.id
                panel.currentNotificationScope = saved.scope
                panel.currentNotificationMatchType = saved.matchType
                panel.currentNotificationMatchValue = saved.matchValue
                panel.currentNotificationVariantMode = saved.variantMode
    ...
    function panel:RefreshItem(itemID, requestID)
        ...
        if realmDB and realmDB.NotificationRequests then
            local function RequestMatchesItem(candidate)
                if not candidate then return false end
                local candidateItemID
                if candidate.matchType == "itemID" then
                    candidateItemID = tonumber(candidate.matchValue)
                elseif candidate.matchType == "dbKey" and MarketSync.ParseItemIDFromDBKey then
                    candidateItemID = MarketSync.ParseItemIDFromDBKey(candidate.matchValue)
                elseif MarketSync.ResolveItemID then
                    candidateItemID = MarketSync.ResolveItemID(candidate.displayName or candidate.matchValue)
                end
                return tonumber(candidateItemID) == itemID
            end

            local requested = requestID and realmDB.NotificationRequests[requestID] or nil
            if RequestMatchesItem(requested) then
                req = requested
            else
                for _, r in pairs(realmDB.NotificationRequests) do
                    if RequestMatchesItem(r) then
                        req = r; break
                    end
                end
            end
        ...
        if req then
            self.currentNotificationRequestID = req.id
            self.currentNotificationScope = req.scope or "all"
            self.currentNotificationMatchType = req.matchType
            self.currentNotificationMatchValue = req.matchValue
            self.currentNotificationVariantMode = req.variantMode
            ...
            self.currentNotificationRequestID = nil
            self.currentNotificationScope = "all"
            self.currentNotificationMatchType = nil
            self.currentNotificationMatchValue = nil
            self.currentNotificationVariantMode = nil
